...
RGBApixel BMP::get_pixel(int i,int j) const{
  using namespace std;
  bool Warn=false;
  if (i>=Width){
    i=Width-1; Warn=true;
...
BMP::BMP(BMP& Input){
  ...
  SetSize(Input.get_width(),Input.get_height());
...
int BMP::get_height(void){
  return Height;
}


int BMP::get_width(void){
  return Width;
}
...
bool BMP::read_from_file(const char* FileName){
  using namespace std;
  if (!EasyBMPcheckDataSize()){
    if (EasyBMPwarnings){
      cout<<"EasyBMP Error: Data types are wrong size!"<<endl
...
int BMP::get_horizontal_DPI(void){
  if (!XPelsPerMeter){
...
void RangedPixelToPixelCopy(BMP& From,int FromL,int FromR,int FromB,int FromT,
  ...
  if (FromR>=From.get_width()){
    FromR=From.get_width()-1;
  }
  if (FromL<0){ FromL=0; }

  if (FromB>=From.get_height()){
    FromB=From.get_height()-1;
  }
  if (FromT<0){ FromT=0; }

  if (ToX+(FromR-FromL)>=To.get_width()){
    FromR=To.get_width()-1+FromL-ToX;
  }
  if (ToY+(FromB-FromT)>=To.get_height()){
    FromB=To.get_height()-1+FromT-ToY;
  }
...
bool Rescale(BMP& InputImage,char mode,int NewDimension){
  ...
  int OldWidth=OldImage.get_width();
  int OldHeight=OldImage.get_height();
